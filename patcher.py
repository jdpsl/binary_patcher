...
def patch_binary(original_file, patch_file, output_file, show_original_jump=False):
    try:
        ...
        # Print original jump location if requested
        if show_original_jump:
            print(f"[I] Original Jump Location: 0x{jump_offset:08X}")
            return  # Exit function after printing
...
if __name__ == "__main__":
    ...
    # Add a flag to show original jump location
    parser.add_argument("-i", "--show-original-jump", action="store_true", help="Show the original jump location and exit.")
    ...
    patch_binary(args.original_file, args.patch_file, args.output_file, args.show_original_jump)
